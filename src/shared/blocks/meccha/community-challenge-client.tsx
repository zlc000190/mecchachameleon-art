...
import { Heart, ImagePlus, Loader2, UploadCloud } from 'lucide-react';

import type { AtlasMap } from '@/shared/blocks/meccha/atlas-data';
import type { CommunityChallenge } from '@/shared/blocks/meccha/community-challenges';
...
export function CommunityChallengeClient({ maps }: { maps: AtlasMap[] }) {
  ...
  async function submitChallenge(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage('Challenge submitted. It is live now; the agent can remove unsuitable entries later.');
      form.reset();
  ...
  async function likeChallenge(id: string) {
    ...
    try {
      await fetch(`/api/community/challenges/${id}`, { method: 'POST' });
    } catch {
    ...
    <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
      <section className="rounded-2xl border border-[#E8D7CC] bg-white p-5 shadow-sm md:p-7">
        ...
        <p className="mt-3 text-sm leading-6 text-[#4C3B35]">
          Pick the map, upload a Meccha Chameleon screenshot, and tell players why this hiding spot survived 30 minutes.
          Submissions are public immediately for this demo; unsuitable entries can be removed by the agent review endpoint.
        </p>
            ...
            <article className="mt-4 overflow-hidden rounded-xl border border-[#E8D7CC] bg-white">
              ...
              <div className="p-4">
                ...
                <button
                  ...
                  className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-lg bg-[#FF6F9A] px-4 text-sm font-black text-white hover:bg-[#E95A88]"
                >
        ...
        <div className="rounded-2xl border border-[#E8D7CC] bg-white p-5 shadow-sm md:p-6">
          ...
          <div className="mt-4 space-y-3">
            {challenges.slice(1, 8).map((challenge) => (
              <div key={challenge.id} className="flex gap-3 rounded-lg border border-[#E8D7CC] p-3">
                <Image src={challenge.screenshotUrl} alt={challenge.title} width={160} height={128} className="h-16 w-20 rounded-md object-cover" />
                <div>
                  <div className="font-black text-[#29211D]">{challenge.title}</div>
