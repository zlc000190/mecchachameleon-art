...
const stepLocales = ['en', 'zh', 'ru', 'es', 'de', 'pt', 'fr', 'it', 'nl', 'ar', 'ja', 'ko'] as const;
...
const copy: Record<SupportedLocale, SectionCopy> = {
  ...
  ko: {
    eyebrow: '멀티플레이',
    title: '친구들을 같은 라운드에 어떻게 모을지',
    intro: 'Meccha Chameleon 은 친구와 함께하는 파티 게임입니다. 친구 그룹, 원거리 크루, 퍼블릭 매치메이킹까지 실전 플레이북을 한 페이지에 정리했습니다.',
    watchEyebrow: '플레이 전 시청',
    watchTitle: 'Meccha Chameleon 입문자 가이드 영상 시리즈',
    watchBody: '첫 영상부터 시작해 재생목록이 자동 진행되도록 두면, 그룹이 페이지를 떠나지 않고 보고 테스트할 수 있습니다.',
    watchBullets: ['1. 게임 루프와 조작 학습.', '2. 하이더/시커 기본 확인.', '3. 다음 라운드까지 탭을 열어두기.'],
    watchFooterLeft: '재생목록이 자동으로 이어집니다.',
    watchFooterRight: '임베드는 사이트에 그대로 유지됩니다.',
    shareTitle: '크루에게 공유하기',
    shareBody: '친구 그룹이 이미 쓰는 플랫폼을 선택하세요. 로그인·가입 없이 링크만 채팅에 던지면 됩니다.',
    nativeShare: '공유',
    shared: '공유됨!',
    unsupported: '지원되지 않음',
    copyLink: '링크 복사',
    copied: '복사됨',
    shareWhyTitle: '왜 공유 버튼이 있나요?',
    shareWhyBody: '커스텀 룸은 친구 4명 이상일 때 재밌습니다. "오늘 밤 해보자" 한 줄짜리 메시지라도, 바로 플레이/시청/스팟 비교가 가능한 페이지로 보내는 게 훨씬 잘 먹힙니다.',
    rosterTitle: '크루 명단 + 초대 스코어보드',
    rosterBody: '누구를 초대했고, 누가 실제로 왔는지, 누가 스코어보드를 끌고 가고 있는지 기록하세요. 이 브라우저에만 저장되고 서버에는 전송되지 않습니다.',
    namePlaceholder: '친구 닉네임 / 핸들',
    add: '추가',
    noCrew: '아직 크루가 없습니다. 커스텀 룸에 부르고 싶은 친구를 먼저 추가하세요. 초대 시 +1, 라운드를 캐리하면 +5.',
    scoreTip: '팁: 스코어는 솔직하게 적으세요. 파티 게임의 절반은 그룹에서 가장 약한 하이더를 찾아 한 주일 동안 놀리는 재미입니다.',
    statusLabel: '상태',
    statuses: { invited: '초대됨', joined: '참여', ghosted: '잠수' },
    platforms: { PC: 'PC', Discord: 'Discord', Mobile: '모바일', Other: '기타' },
    ctaTitle: '큐에 들어가기 전에 완전 초보 가이드가 더 필요한가요?',
    ctaBody: '조작법, 페인트 도구, 역할 가이드, 첫 매치 체크리스트를 한 페이지에 정리했습니다.',
    ctaGuide: '초보자 가이드',
    ctaPlay: '온라인으로 플레이',
    shareText: 'Meccha Chameleon 브라우저 플레이 + 멀티플레이 가이드를 한 페이지에 정리한 링크, 그룹 채팅에 바로 던지기 좋습니다:',
    pageTitle: 'Meccha Chameleon 한국어',
    modes: [
      { name: '클래식 숨바꼭질', players: '2 - 24', body: '하이더가 페인트 후 굳음, 시커가 사냥. 기본 루프 학습에 최적.' },
      { name: '인펙션', players: '4 - 12', body: '발견된 플레이어가 시커가 됨. 라운드마다 긴장감 증가.' },
      { name: '스피드 헌트', players: '3 - 8', body: '하이더가 동시에 페인트 후, 스캔 속도가 승부를 가름.' },
      { name: '커스텀 룸', players: '2 - 24', body: '친구 그룹 전용 프라이빗 로비. 긴 세션과 맵 연습에 최고.' },
    ],
    steps: [
      { title: '같은 방, 같은 Wi-Fi', body: '가장 단순한 셋업: 한 명이 호스트, 나머지는 초대로 입장. 포트 포워딩, 방화벽 설정 불필요.', bullets: ['호스트가 커스텀 룸 오픈.', '게임 내 초대 기능으로 친구 초대.', '음성 채팅으로 시커가 좌표 콜아웃.'] },
      { title: '다른 집, 같은 국가', body: '커스텀 룸은 릴레이 서버를 통해 동작하므로 다른 도시·지역 친구도 참여 가능.', bullets: ['가장 가까운 릴레이 지역 선택.', '80ms 이하 쾌적, 140ms 이하도 플레이 가능.', '회사망이 까다로우면 모바일 핫스팟이 빠름.'] },
      { title: '대륙을 가로지르는 경우', body: '원거리 그룹은 호스트와 친구 사이 중간의 릴레이 지역 선택.', bullets: ['시작 전 ping 비교.', '지연이 느껴지면 네트워크 통계 활성화.', '음성은 외부 도구로 분리하고, 매치는 클라이언트에서 진행.'] },
      { title: '퍼블릭 매치메이킹', body: '퍼블릭 큐가 진입 장벽이 가장 낮음. 고정 크루 만들기 전 루프를 익히는 최단 거리.', bullets: ['2 - 10 명이 권장 인원.', '큐 전에 언어 설정.', '고수 로비는 Discord 의 커스텀 룸에서 형성.'] },
      { title: '스트리밍 & 와치파티', body: '페인트 단계와 리빌 단계 모두 스트리밍 친화적.', bullets: ['1080p / 60 면 충분.', '리빌 전에 클립 단축키 설정.', '채팅이 스나이핑하면 로비 이름 표시 끄기.'] },
      { title: '가족/캐주얼 친구와 함께', body: '스킬 플로어가 낮아서, 초보자도 한 라운드만 설명받으면 바로 합류 가능.', bullets: ['Mac 네이티브 클라이언트 없음.', '모바일 네이티브 버전 없음, PC 스트리밍 권장.', '푸시 투 토크와 페인트 도구는 한 라운드면 전수 가능.'] },
    ],
  },
  en: {
    eyebrow: 'Multiplayer',
